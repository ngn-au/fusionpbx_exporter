...
var (
	...
	missedCalls   = make(map[string]prometheus.Gauge)
...
func CollectMetrics() {
	...
	rows, err = db.Query("SELECT d.domain_name, COUNT(*) FROM v_domains d JOIN v_xml_cdr c ON d.domain_uuid = c.domain_uuid WHERE c.hangup_cause = 'ORIGINATOR_CANCEL' AND c.end_stamp > clock_timestamp() - INTERVAL '30 seconds' GROUP BY d.domain_name")
	if err != nil {
		log.Fatal(err)
	}
	...
	for rows.Next() {
		var domain string
		var missedCallsCount sql.NullFloat64
		if err := rows.Scan(&domain, &missedCallsCount); err != nil {
			log.Fatal(err)
		}

		gauge, ok := missedCalls[domain]
		if !ok {
			gauge = prometheus.NewGauge(prometheus.GaugeOpts{
				Name:        "missed_calls_per_domain",
				Help:        "Number of missed calls per domain",
				ConstLabels: prometheus.Labels{"domain": domain},
			})
			reg.MustRegister(gauge)
			missedCalls[domain] = gauge
		}

		gauge.Set(missedCallsCount.Float64)
	}
	...
	for rows.Next() {
		...
		if !ok {
			gauge = prometheus.NewGauge(prometheus.GaugeOpts{
				...
				Help:        "Average call duration per domain",
				ConstLabels: prometheus.Labels{"domain": domain},
			})
			reg.MustRegister(gauge)
